...
class User(db.Model):
    ...
    analyzed_channels_rel = db.relationship('UserChannels', back_populates='user', lazy=True)
...
# Link users to channels they have analyzed (Restored from HEAD, adapted)
class UserChannels(db.Model):
    ...
    user = db.relationship('User', back_populates='analyzed_channels_rel')
    channel = db.relationship('Channel', backref=db.backref('analyzed_by', lazy=True))
